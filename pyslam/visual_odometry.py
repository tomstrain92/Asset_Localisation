...
class VisualOdometry(object):
    ...
    def processFirstFrame(self):
        ...
        self.draw_img = self.drawFeatureTracksAndVehicles(self.cur_image)
    ...
    def processFrame(self, frame_id):
        ...
        # this is the frame to frame update
        self.R = Rotation.from_matrix(R) # this will help with linearisation later.
        self.t = t.reshape(-1)
        ...
        if(absolute_scale > kAbsoluteScaleThreshold):
            ...
            # I have changed this to be scipy rotation object - this to help
            # with linearisation when updating all particle rotations later on by
            # rot_particles_i+1 = rot_particles_i * self.R

            self.cur_t = self.cur_t + absolute_scale*self.cur_R.apply(self.t)
            self.cur_R = self.cur_R * self.R
    ...
    def drawFeatureTracksAndVehicles(self, img, reinit = False):
            ...
            if reinit:
                ...
                for i,pts in enumerate(zip(self.track_result.kp_ref_matched, self.track_result.kp_cur_matched)):
                    ...
                    if self.mask_match[i] or drawAll:
                        ...
                        num_outliers+=1
                # draw vehicles
                for box in self.feature_tracker.vehicle_boxes:
                    x,y,w,h = box
